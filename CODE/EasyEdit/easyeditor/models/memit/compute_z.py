...
def compute_z(
    ...
    rewriting_targets = torch.tensor(-100, device=default_device).repeat(
    ...
    if hasattr(model.config, 'n_embd'):
        delta = torch.zeros(
            (model.config.n_embd,), requires_grad=True, device=default_device
        )
    elif hasattr(model.config, 'hidden_size'):
        delta = torch.zeros(
            (model.config.hidden_size,), requires_grad=True, device=default_device
        )
    else:
