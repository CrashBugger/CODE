...
def layer_stats(
    ...
    if (
        batch_size is None
        and hparams is not None
        and hasattr(hparams, "mom2_batch_size")
    ):
        batch_size = hparams.mom2_batch_size

    if (
        batch_tokens is None
        and hparams is not None
        and hasattr(hparams, "mom2_batch_tokens")
    ):
    ...
    def get_ds():
        ...
        # raw_ds = {'train': raw_ds}
        raw_ds = load_dataset(
            "wikimedia/wikipedia" if ds_name == "wikipedia" else ds_name,
    ...
    if hasattr(model, "get_input_embeddings") and model.get_input_embeddings() is not None:
        default_device = model.get_input_embeddings().weight.device
    else:
    ...
    with torch.no_grad():
        for batch_group in progress(loader, total=batch_count):
            for batch in batch_group:
                batch = dict_to_(batch, default_device)
                with Trace(
