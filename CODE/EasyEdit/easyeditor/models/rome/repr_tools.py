...
def get_reprs_at_idxs(
    model: AutoModelForCausalLM,
    tok: AutoTokenizer,
    contexts: List[str],# Complete sentence representing the knowledge
    idxs: List[List[int]],# Position of the filled word
    layer: int,
    module_template: str,
    track: str = "in",
) -> torch.Tensor:
    ...
    def _batch(n):
        for i in range(0, len(contexts), n):
            yield contexts[i : i + n], idxs[i : i + n]# Chunk sentences and filled word positions
    ...
    tin, tout = (
        (track == "in" or both),
        (track == "out" or both),
    )# tin and tout are both bool structures
