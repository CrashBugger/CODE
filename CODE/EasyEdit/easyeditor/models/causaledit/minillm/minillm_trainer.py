...
def get_config_model_id(config):
...
class MiniLLMTrainer(GRPOTrainer):
    ...
    def _single_step_decomposition_loss(
        ...
        if reduction == "batchmean":
            return (
                reg_loss.sum() / mask.sum()
                if mask is not None
                else reg_loss.sum() / reg_loss.size(0)
            )
        elif reduction == "sum":
            return reg_loss.sum()
        elif reduction == "mean":
    ...
    def _compute_advantage(
        ...
        if self.gamma > 0.0:
            gamma_pow = torch.pow(
                self.gamma, torch.arange(response_length, device=rewards.device)
            )
    ...
    def compute_loss(
        ...
        if self.rkl_advantage:
            ...
            inputs["advantages"] = (
                inputs["advantages"].unsqueeze(1) + reverse_kl_advantage
            )
