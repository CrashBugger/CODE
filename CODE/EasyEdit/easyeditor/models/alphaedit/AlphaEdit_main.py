...
def apply_AlphaEdit_to_model(
    ...
    **kwargs,
    ...
    P_path = os.path.join(
        hparams.P_loc, f"{hparams.model_name.replace('/', '_')}_null_space_project.pt"
    )
    if not os.path.exists(P_path):
        logger.info(os.path.abspath(hparams.P_loc))
        logger.info(
            "The null-space projection matrix P does not exist and now calculate."
        )
        ...
        if (
            ...
            raise ValueError(
                f"Unknown model family for null-space project init: {hparams.model_name}. "
                "Please add shape handling for this model."
            )
        del W_out
        ...
        # Modified
        torch.save(P, P_path)
        logger.info(f"The null-space projection matrix P is saved to {P_path}")
        # Modified
        P_loaded = True
    elif P_loaded == False:
        P = torch.load(P_path)
    ...
    logger.info(f"New weights successfully inserted into {list(deltas.keys())}")
...
def execute_AlphaEdit(
    ...
    for request in requests:
        ...
        if (
            ...
            try:
                ...
                logger.warning(f"Error reading cache file due to {e}. Recomputing...")
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        logger.info(f"Writing {layer_ks.size(1)} key/value pair(s) into layer {layer}")
        ...
        targets = zs - cur_zs
        logger.info(f"z error {torch.linalg.norm(targets, dim=0).mean()}")

        repeat_factor = layer_ks.size(1) // targets.size(1)
        targets = targets.repeat_interleave(repeat_factor, dim=1)
        resid = targets / (len(hparams.layers) - i)  # Distribute residual across layers
        solve_dtype = torch.float32
        proj = P[i, :, :].to(f"cuda:{hparams.device}", dtype=layer_ks.dtype)
        lhs = (
            proj
            @ (
                layer_ks.to(f"cuda:{hparams.device}")
                @ layer_ks.T.to(f"cuda:{hparams.device}")
                + cache_c[i, :, :].to(f"cuda:{hparams.device}", dtype=layer_ks.dtype)
            )
            + hparams.L2
            * torch.eye(
                layer_ks.shape[0],
                dtype=layer_ks.dtype,
        ...
        rhs = (
            proj
            @ layer_ks.to(f"cuda:{hparams.device}")
            @ resid.T.to(f"cuda:{hparams.device}", dtype=layer_ks.dtype)
        ).to(solve_dtype)
        upd_matrix = torch.linalg.solve(lhs, rhs).to(layer_ks.dtype)

        # Adjust update matrix shape
        weight_name = f"{hparams.rewrite_module_tmp.format(layer)}.weight"
        upd_matrix = upd_matrix_match_shape(upd_matrix, weights[weight_name].shape)

        logger.info(f"orig norm {torch.linalg.norm(weights[weight_name])}")
        logger.info(f"upd norm {torch.linalg.norm(upd_matrix)}")
...
def get_cov(
    ...
    logger.info(f"Retrieving covariance statistics for {model_name} @ {layer_name}.")
...
def get_context_templates(model, tok):
    ...
    if CONTEXT_TEMPLATES_CACHE is None:
        ...
        logger.info(f"Cached context templates {CONTEXT_TEMPLATES_CACHE}")
...
def get_project(model, tok, layer, hparams):
    ...
    logger.info(f"small_singular_indices count: {len(small_singular_indices)}")
