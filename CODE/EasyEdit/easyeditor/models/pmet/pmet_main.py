...
def apply_pmet_to_model(
    ...
    **kwargs
    ...
    deltas = execute_pmet(model, tok, requests, hparams, cache_template=cache_template) # Stores the left and right values of Eq14
    ...
    with torch.no_grad():
        for w_name, upd_matrix in deltas.items(): #w_name, adj_k, resid
            ...
            w[...] += upd_matrix.float() # w[...] advanced indexing, operates on each element of w
