...
def tokenize(batch, tokenizer, device, context_templates=None, hparams=None):
    ...
    for i, loc_prompt in enumerate(loc_prompts):
        if loc_prompt in prompts[i]:  # subject: Factual Editing
            ...
            mask_shape = tokens['input_ids'][int(i*len_temp):int((i+1)*len_temp)].shape
            act_mask = torch.zeros(mask_shape, dtype=torch.bool)
            deact_mask = torch.zeros(mask_shape, dtype=torch.bool)
...
def get_context_templates(model, tok, length_params, device):
    ...
    if CONTEXT_TEMPLATES_CACHE is None:
        ...
        for length, n_gen in length_params: 
            ...
            decoded = tok.batch_decode(gen_token, skip_special_tokens=True)
            decoded = [text.replace('{', '{{').replace('}', '}}') for text in decoded]
            CONTEXT_TEMPLATES_CACHE += decoded
